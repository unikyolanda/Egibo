...
function FormSuccessPage() {
    ...
    useEffect(() => {
        if (email) {
            const handleDownload = async () => {
                try {
                    const response = await axios.post('http://125.228.62.164:5006/create_report', { email })
    ...
    return (
        <div className="bg-[#f7f7f9] w-full h-screen flex flex-col justify-center items-center">
            ...
            {url ? (
                <button className='rounded-full w-[250px] sm:w-[397px] py-4 text-base sm:text-lg flex justify-center font-medium bg-[#4339e4] text-white hover:bg-[#2e26a3] mb-4'
                    onClick={() => url && window.open(url, '_blank')}>下載 PDF 文件</button>
            ) : (
                <div className='z-10 w-[250px] sm:w-[397px] py-4 text-base sm:text-lg flex justify-center font-medium text-[#101828] mb-4'
                    disabled>PDF文件產生中，請稍候...</div>
            )}
            <Link to='/home'>
